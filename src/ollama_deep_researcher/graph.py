...
from ollama_deep_researcher.task_pdf import compare_pdf, ingest, summary_pdf, extract_pdf_name, get_abstract
...
def router(state: SummaryState, config: RunnableConfig) -> Literal["upload_pdf", "generate_query", "compare_pdf"]:
    request = state.research_topic
    condition_str1 = "upload"
    condition_str2 = "compare"
    if condition_str1 in request:
        return "upload_pdf"
    else:
...
def upload_pdf(state: SummaryState, config: RunnableConfig):
    # docs = []
    research_topic = state.research_topic
    ...
    return {"paper": state.paper, "abstract": state.abstract}

def compare_pdf(state: SummaryState, config: RunnableConfig):
    return "Here is the comparison between two papers"
...
builder.add_node("upload_pdf", upload_pdf)
builder.add_node("compare_pdf", compare_pdf)
...
builder.add_edge("upload_pdf", END)
builder.add_edge("compare_pdf", END)
